# Vendored CAD runtime workflow

Provenance: maintained in [earthtojake/text-to-cad](https://github.com/earthtojake/text-to-cad).
This is an attributed, app-owned runtime guide rather than an installable agent
skill. The repository link is only for provenance and release review.
...
Use this workflow for CAD files, STEP/STP files, build123d source, selector refs such as `#o1.2.f1`, mechanical parts, assemblies, enclosures, brackets, fixtures, holes, counterbores, countersinks, slots, pockets, bosses, standoffs, ribs, fillets, chamfers, shells, source-level joints, mating, or measurements. It also applies when the user supplies reference images or 2D technical drawings of a part to reproduce or take design intent from.

It also covers STL, 3MF, or native GLB output from CAD geometry. Keep those workflows secondary and load `references/supported-exports.md` for details.

Do not use this workflow for render-only concept art, CAM toolpaths, engineering certification, FEA conclusions, architectural BIM, or freehand illustration unless the user also needs CAD geometry.
...
From the `cad-runtime` directory, the launcher shape is:
...
4. **Check named purchasable components.** When an assembly includes named off-the-shelf actuators, servos, motors, electronics boards, connectors, or other purchasable components, use an approved component source when one is available. If no exact match is found, record the miss and use a documented envelope.
...
9. **Review the primary STEP snapshot.** After creating or visibly updating a primary STEP/STP part or assembly, wait for and review the explicit `snapshot.png` captured by the CAD app's embedded viewer. Do not launch browser automation. Deterministic checks passing is not a reason to skip visual review; if no snapshot arrives within the bounded wait, report the run as waiting.
...
Publish generated source, STEP, GLB, snapshot, topology, and validation outputs
through the host Files surface. Update the app's generation entity with the
exact Files references and checks that actually ran. The embedded CAD viewer,
not an external viewer process, owns visual presentation.
...
Final reports should include generated files, verification snapshots,
validation actually run, assumptions, and caveats. Use
`references/inspection-and-validation.md` for report structure.
