...
export const ICONS = {
  ...
  // ISPO PORT: back to the deck library.
  //
  // A CHEVRON, not a shelf. The first version was a 2x2 grid, which is a fine
  // symbol for "a library" and a bad one for "leave this deck" — reported as
  // "there's no back button" while the button sat two icons from the reader's
  // cursor. In a bar that is nothing but icons, the thing that means BACK has
  // to look like every other back control they have ever used. Paired with a
  // label the compact bar is not allowed to drop (styles.css).
  decks: svg('<polyline points="14 6 8 12 14 18"/>'),
