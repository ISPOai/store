...
// ISPO PORT: window.prompt is inert in a sandboxed project frame (no
// `allow-modals`) — it returns null without showing anything, which would make
// commenting silently impossible. See ispo/src/dialogs.ts.
import { askText } from '../../../ispo/src/dialogs.ts'

/** The commenter's name, remembered per browser (localStorage, never sent
 *  anywhere); asked for on first use. ISPO: async, because the ask is now a
 *  dialog element rather than window.prompt. */
export async function commentAuthor(): Promise<string | null> {
  const stored = lsGet('bento-author')
  if (stored) return stored
  const name = (await askText(t('Your name (shown on comments):'), '', { ok: t('Save') }))?.trim()
  if (!name) return null
...
export async function changeCommentAuthor(): Promise<string | null> {
  const next = (await askText(
    t('Your name (shown on new comments):'), lsGet('bento-author') ?? '', { ok: t('Save') },
  ))?.trim()
  if (!next) return null
...
export class CommentsUI {
  ...
  async openNew(elementId?: string, point?: { x: number; y: number }) {
    ...
    if (!author) return
    const text = (await askText(t('Comment:'), '', { ok: t('Comment'), multiline: true }))?.trim()
    if (!text) return
  ...
  private openThread(commentId: string, anchor: HTMLElement) {
    ...
    me.addEventListener('click', () => {
      void changeCommentAuthor().then((next) => {
