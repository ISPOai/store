...
import { APP_VERSION, applyUpdate, applyUpdateInPlace, autoCheckEnabled, canUpdateInPlace, checkForUpdates, compareVersions, offlineEnabled } from '../update'
...
// ISPO PORT: window.alert/confirm/prompt are inert in a sandboxed project frame
// (no `allow-modals`) — confirm() returns false and prompt() returns null with
// nothing shown, so a destructive-action guard becomes "you can never do this".
// See ispo/src/dialogs.ts for the substitutes and why they are async.
import { ask, askText, say, chooseFrom } from '../../../ispo/src/dialogs.ts'
import { currentDeck, deckChangedOnDisk, listDecks, readDeck, refFor, setCurrentDeck } from '../../../ispo/src/store.ts'
import { createDeck, importDeck } from '../ispo/deck-actions'
import { exportDeckCommand } from '../ispo/commands'
import { showLibrary } from '../ispo/library'
...
import { joinFromDoc, mintCollab, mintInvite, onlineTransport, rotateKeys, sharingOn, startSharing, stopSharing } from '../sync/online'
...
export class Editor {
  ...
  private build() {
    ...
    // ISPO PORT: the way back to the deck library, and the first thing in the
    // bar. A left chevron with a label the compact bar is not allowed to drop —
    // in a row that is otherwise all icons, the control that means BACK has to
    // look like every back control the reader has ever used.
    const decksB = btn(ICONS.decks, t('Decks'),
      () => void showLibrary(), t('Back to all your decks'))
    decksB.classList.add('ed-decks-btn')
    decksB.setAttribute('aria-label', t('Back to all your decks'))
    ...
    // ISPO PORT: no wordmark in the bar. Inside ISPO the app's identity is already
    // the window: the host paints the project's name and icon in its own chrome,
    // so repeating it here spends the scarcest row in the app on a second copy.
    // What that row owes the reader instead is the way OUT, which is why `decksB`
    // leads and carries a gap after it (styles.css `.ed-decks-btn`).
    bar.append(decksB, this.updatesB, title, this.fileChip, slidesB, insertD, history, insert, actions, moreD)
  ...
  private buildSaveAsItems(into: HTMLElement, close: () => void, mark = false) {
    ...
    }
    {
      ...
      // ISPO PORT: upstream has no Open and no New — one file is one deck, and
      // the file manager is the library. Here the library is the project's own
      // deck storage, so it needs a way in and a way to start a second deck.
      item(ICONS.copy, t('Open deck…'),
        t('Switch to another deck in this app, or bring one in from your Files.'),
        () => void this.openDeckPicker())
      item(ICONS.plus, t('New deck'),
        t('Start an empty deck. This one stays where it is.'),
        () => void this.newDeckFromLibrary())
      item(ICONS.copy, t('Export to Files…'),
        t('Hand this deck to your Files library, where you can share it or open it elsewhere.'),
        () => void this.exportDeck())
      ...
      // ISPO PORT: About lived on the wordmark, and the wordmark is gone from
      // the bar. It carries version, interface language, theme, licenses and
      // document properties, so it needed a door that always exists — this menu.
      into.appendChild(tag(div('ed-menu-sep')))
      item(ICONS.globe, t('About Bento Slides'),
        t('Version, interface language, theme, licenses and document properties.'),
        () => this.openAbout())
    }
  }
  ...
  private async startFromScratch() {
    ...
    if (!(await ask(t('Replace all {n} slides with one blank slide? ⌘Z undoes this.', { n: String(n) }),
      { ok: t('Replace'), danger: true }))) return
  ...
  private renderSharePanel() {
      ...
      for (const peer of peers) {
        ...
        if (iAmOwner && peer.pub && peer.pub !== cme!.owner) {
          ...
          kick.addEventListener('click', async (ev) => {
            ev.stopPropagation()
            if (!(await ask(t('Remove {name} from this deck? Their copy drops to read-only.', { name: peer.name }), { ok: t('Remove'), danger: true }))) return
            const tr = onlineTransport()
    ...
    if (offlineEnabled()) {
      // ISPO PORT: offline is pinned on (kernel/src/net.ts), so the second line
      // upstream shows here — "turn offline mode off to collaborate online" —
      // would point at a switch that cannot move. Say what is actually true.
      note(t('Offline mode is on — nothing leaves this computer.'))
      note(t('This app is installed from the ISPO Store with no network access, so live collaboration is unavailable. Share a deck by exporting it.'))
      return
    }
    ...
    if (canWrite) {
      ...
      action(ICONS.key, t('Reset access…'), false, async () => {
        if (!this.session) return
        if (!(await ask(t('Reset access? Every copy you’ve sent stops syncing; only copies saved after this can join.'), { ok: t('Reset'), danger: true }))) return
  ...
  private makeThumb(slide: import('../model').Slide, i: number, isState: boolean): HTMLElement {
    ...
    tools.append(
      ...
      btn(ICONS.trash, '', (ev) => { ev.stopPropagation(); void this.deleteSlide(i) }, t('Delete slide')),
    )
  ...
  private async deleteSlide(i: number) {
    ...
    if (states.length || linkCount) {
      ...
      if (!(await ask(t('Delete this slide? {parts}.', { parts }), { ok: t('Delete'), danger: true }))) return
    }
    this.store.commit(() => {
  ...
  private async promptMediaUrl(kind: 'video' | 'audio') {
    ...
    const url = (await askText(
      t('Paste the {kind} URL — it stays a link, the file is not embedded:', { kind: t(kind) }),
      '', { ok: t('Insert'), placeholder: 'https://…' },
    ))?.trim()
    if (!url) return
  ...
  private pickMedia() {
    ...
    input.addEventListener('change', () => void (async () => {
      ...
      if (file.size > MEDIA_EMBED_BUDGET) {
        ...
        const ok = await ask(t(
          'This {kind} is {mb} MB. Embedding keeps it inside the deck but makes it large and slow to open and save. Embed anyway? Cancel, then paste a hosted URL in the panel to keep the deck small.',
  ...
  private async runAutosave() {
    ...
    // No deck file yet (a deck imported from Files has no save target until the
    // author names one) or the write failed: what is on disk is STALE and the
    // deck stays dirty — saying "Saved" here would be a lie. But the snapshot
    // means the work is not lost, and that was previously invisible: nothing
    // was shown at all, so the only signal was an amber dot that never cleared.
    // Say what is actually true.
    //
    // Deliberately silent for an ENCRYPTED deck: those are never snapshotted
    // (that would write plaintext), so there is no backstop to claim.
    //
    // ISPO PORT: the snapshot lands in this project's own private storage, not
    // in a browser origin — so it survives a cleared browser and travels with
    // the project. The copy says where it is, because "in this browser" would
    // now point at the wrong place.
    if (snapshotted) {
      this.lastBackupAt = Date.now()
      this.flashSaved(t('Backed up in this app’s storage'))
  ...
  private refreshDirtyHint() {
    // ISPO PORT: the question is "is there a deck file to write to?", not "can
    // this browser write in place?" — `canWriteInPlace()` is always true here,
    // so the upstream guard would silence the hint in exactly the state that
    // needs it (a deck imported from Files, not yet given a name).
    if (hasFileHandle() || !this.lastBackupAt) return
    const when = new Date(this.lastBackupAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
    this.dirtyDot.title = t('Unsaved changes — kept in this app’s storage at {when} and offered back if you reopen. ⌘S writes them to a deck.', { when })
  }
  ...
  private async openDroppedDeck(ev: DragEvent): Promise<boolean> {
    ...
    if (this.store.dirty && !(await ask(t('Open {name}? Unsaved changes in this deck will be lost.', { name: named }), { ok: t('Open') }))) return true
    ...
    // a foreign file.
    if (el && !block) { await say(t('{name} is an empty copy of Bento, not a saved deck. Open it on its own to start one.', { name: named })); return true }
    let parsed: unknown
    try { parsed = JSON.parse(block) } catch { await say(t('{name} isn’t a Bento document.', { name: named })); return true }
    if ((parsed as { format?: string })?.format === 'bento/enc') {
      await say(t('{name} is password-protected. Open it directly to unlock it.', { name: named }))
      return true
    }
    const next = parseDoc(JSON.stringify(parsed))
    if (!next) { await say(t('{name} isn’t a Bento document.', { name: named })); return true }
  ...
  /**
   * ISPO: adopt a change another writer made to the open deck.
   *
   * This project exports HEADLESS COMMANDS (add-slide, new-deck) that a host
   * or an agent can invoke while the editor is on screen; they write the deck
   * JSON straight to disk. Without this the editor would keep rendering the
   * document it loaded and its next save would overwrite that work — a silent
   * loss caused by the app's own command surface, which is worse than not
   * having one.
   *
   * Checked when the frame comes back to the foreground rather than on a
   * timer: an ISPO project frame gets no rendering steps while it is hidden
   * (backgrounded iframes drop rAF and ResizeObserver alike), so a poll would
   * not run there anyway, and coming back IS the moment the answer matters.
   *
   * THREE signals, because no one of them covers the case:
   *   · `ispo:focus-project` — the host telling this project it is now the
   *     active pane. The only signal that fires when the user switches panes
   *     inside a window that never lost visibility, which is the common case.
   *   · `focus` — the window coming back from another app.
   *   · `visibilitychange` — the window being restored or the tab re-shown.
   * There is deliberately NO `visibilityState` guard: it would silence the
   * first signal (a pane switch leaves the document visible throughout) and
   * a single small read costs nothing when it finds no change.
   *
   * An edit in progress always wins — a dirty document is never replaced under
   * the author's hands. The disk copy is not lost by that: it is read again on
   * the next check, and the author is told rather than merged with.
   */
  watchForExternalDeckChanges(): void {
    const check = () => {
      void deckChangedOnDisk().then((body) => {
        if (!body) return
        if (this.store.dirty) {
          this.toast(t('This deck also changed outside the editor — save or undo to see those changes.'))
          return
        }
        const next = parseDoc(body)
        if (!next) return
        this.store.replaceDoc(next)
        this.canvas.render()
        this.syncWindowTitle()
        this.toast(t('Reloaded — this deck changed outside the editor.'))
      })
    }
    // 'ispo:focus-project' is the SDK's DOM_EVENT_FOCUS_PROJECT, dispatched on
    // `window` when the host makes this project the focused pane. Listened for
    // by NAME rather than by importing the constant so this file keeps
    // compiling if the SDK is not resolvable (it is a string in the wire
    // contract, not a version-bound symbol).
    window.addEventListener('ispo:focus-project', check)
    document.addEventListener('visibilitychange', check)
    window.addEventListener('focus', check)
  }

  // --- ISPO: the deck library --------------------------------------------
  //
  // Upstream has neither of these, because upstream does not need them: one
  // file is one deck and the operating system is the library. Here decks live
  // in the project's own private storage, so switching between them and
  // starting a second one are things only this app can offer.

  /** Switch decks, or bring one in from the user's Files library. */
  private async openDeckPicker(): Promise<void> {
    ...
    const picked = await chooseFrom(
      ...
      decks.map((d) => ({
        id: d.path,
        label: fileBase(d.name),
        hint: d.path === open?.path ? t('open') : '',
      })),
      {
        empty: t('No other decks yet.'),
        actions: [{ id: 'import', label: t('From Files…') }],
      },
    )
    if (!picked) return
    if (picked.kind === 'action') return this.importDeckFromLibrary()
    if (picked.id === open?.path) return
    await this.openDeckPath(picked.id)
  }

  /**
   * Open a deck by its fs path, into THIS editor.
   *
   * Public because the deck library (ispo/library.ts) opens decks too, and it
   * must not construct a second Editor to do it — one editor, one set of
   * document-level listeners. Returns whether the deck actually opened, so the
   * library knows whether to close itself.
   */
  async openDeckPath(path: string): Promise<boolean> {
    if (path === currentDeck()?.path) return true
    if (!(await this.confirmLeavingUnsaved())) return false
    let body: string
    try {
      body = await readDeck(path)
    } catch (err) {
      await say(t('That deck could not be read: {err}', { err: String((err as Error)?.message ?? err) }))
      return false
    }
    if (body.includes('"bento/enc"') && !parseDoc(body)) {
      await say(t('That deck is password-protected. Bento can only unlock an encrypted deck at launch.'))
      return false
    }
    const next = parseDoc(body)
    if (!next) {
      // Deliberately does NOT switch: a deck this build cannot parse is still
      // somebody's data, and leaving it closed leaves it intact.
      await say(t('{name} isn’t a Bento document this version can open. It has been left as it is.',
        { name: path }))
      return false
    }
    setCurrentDeck(refFor(path))
    this.adoptOpenedDoc(next, fileBase(refFor(path).name))
    return true
  }

  /**
   * Read a deck out of Files through the host powerbox.
   *
   * A pick arrives BY COPY with no write-back handle (spec §19.5: editing a
   * pick forks), so the opened deck starts with no save target — the first ⌘S
   * asks where it should live in this app, exactly as a never-saved deck does.
   */
  /** Adopt an already-picked document (the library's import path). */
  async adoptImportedBody(body: string, name: string): Promise<void> {
    const next = parseDoc(body)
    if (!next) return
    if (!(await this.confirmLeavingUnsaved())) return
    setCurrentDeck(null)
    this.adoptOpenedDoc(next, fileBase(name))
  }

  /** Bring a deck in from Files, into this editor. */
  async importDeckFromLibrary(): Promise<void> {
    if (!(await this.confirmLeavingUnsaved())) return
    const picked = await importDeck()
    if (!picked) return
    // A pick arrives BY COPY with no write-back handle (spec §19.5: editing a
    // pick forks), so the opened deck starts with no save target — the first
    // ⌘S asks where it should live, exactly as a never-saved deck does.
    setCurrentDeck(null)
    this.adoptOpenedDoc(picked.doc, fileBase(picked.name))
  }

  /** Start an empty deck, saved straight away so it exists in the library. */
  async newDeckFromLibrary(): Promise<void> {
    if (!(await this.confirmLeavingUnsaved())) return
    const made = await createDeck()
    if (!made) return
    setCurrentDeck(refFor(made.path!))
    this.adoptOpenedDoc(made.doc, made.name)
  }

  /**
   * Publish the deck into this project's Files folder.
   *
   * Runs the SAME handler the host invokes as the `export-deck` command — one
   * implementation, two entry points, so a deck exported by hand and one
   * exported by an agent land in the same place in the same shape.
   *
   * The handler reads the deck FROM DISK, because that is what a headless
   * invocation must do. So an unsaved edit has to be committed first, or the
   * button would quietly publish the previous version of the slide the author
   * is looking at.
   */
  private async exportDeck(): Promise<void> {
    try {
      if (this.store.dirty) {
        ...
        // `save` reports nothing back; a cancelled name prompt leaves the deck
        // dirty. Publishing anyway would hand out the previous version.
        if (this.store.dirty) {
          this.toast(t('Save the deck first — the export publishes what is on disk.'))
          return
        }
      }
      const result = await exportDeckCommand.run({})
      const at = result.refs[0]?.path
      this.toast(at ? t('Exported to {name}', { name: at }) : t('Exported to Files'))
    } catch (err) {
      await say(t('That export failed: {err}', { err: String((err as Error)?.message ?? err) }))
    }
  }

  private async confirmLeavingUnsaved(): Promise<boolean> {
    if (!this.store.dirty) return true
    return ask(t('This deck has unsaved changes. Leave them?'), { ok: t('Leave'), danger: true })
  }

  private adoptOpenedDoc(next: import('../model').BentoDoc, named: string): void {
    this.openedAs = named
    this.store.replaceDoc(next)
    this.canvas.render()
    this.syncWindowTitle()
    this.flashSaved(t('Opened {name}', { name: named }))
  }
  ...
  private openAbout(runCheck = false) {
    ...
    // ISPO PORT: the release channel is gone (kernel/src/update.ts) — ISPO
    // updates this app through the Store — so the auto-check switch has
    // nothing to switch. Shown DISABLED rather than removed: a reader looking
    // for "does this thing phone home at launch?" deserves to find the answer
    // where it has always been, and the answer is no.
    ...
    autoRow.append(autoCb, document.createTextNode(' ' + t('Updates are managed by ISPO — reinstall from the Store to update')))
    box.appendChild(autoRow)

    // ISPO PORT: the hard no-network switch is pinned ON by the host, not by
    // this checkbox — the app declares an empty `egress.connect` envelope, so
    // the host's CSP refuses every outbound request regardless. The control
    // stays visible and disabled because it is the one place a reader looks to
    // answer "can this deck reach the network?", and removing it would leave
    // that question unanswered rather than answered well.
    ...
    offCb.disabled = true
    offRow.append(offCb, document.createTextNode(' ' + t('Offline — this app has no network access at all (enforced by ISPO)')))
