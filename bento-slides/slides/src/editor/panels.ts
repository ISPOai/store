...
import { t } from '../i18n'
// ISPO PORT: window.confirm/prompt are inert in a sandboxed project frame
// (no `allow-modals`). See ispo/src/dialogs.ts.
import { ask, askText } from '../../../ispo/src/dialogs.ts'
...
export class PropsPanel {
  ...
  private buildSlidePanel() {
    ...
    saveLy.addEventListener('click', () => void (async () => {
      const name = await askText(t('Layout name'), this.store.slide.name ?? 'My layout', { ok: t('Save') })
      if (!name) return
      this.edit(() => {
    ...
    replace.addEventListener('click', () => {
      ...
      input.addEventListener('change', () => void (async () => {
        ...
        if (file.size > MEDIA_EMBED_BUDGET) {
          ...
          if (!(await ask(t('This file is {mb} MB. Embedding makes the deck large and slow to open and save. Embed anyway?', { mb }), { ok: t('Embed') }))) return
        }
        const reader = new FileReader()
