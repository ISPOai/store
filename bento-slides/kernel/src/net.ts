...
// Ported to ISPO from nyblnet/bento kernel/src/net.ts (see UPSTREAM.md).
// ISPO change: offline mode is pinned on; see offlineEnabled below.
//
...
export const offlineEnabled = (): boolean => {
  // ISPO PORT: pinned ON. This app is installed from the ISPO store with an
  // EMPTY `egress.connect` envelope, so the host's CSP (`connect-src 'self'
  // assets:`) refuses every outbound request before it leaves the frame.
  // Reporting that truthfully here — rather than letting requests fail against
  // the CSP — is what turns a silent block into the honest "offline" state
  // every affected surface in this app already knows how to render.
  //
  // It also keeps the guarantee the switch was built for: `remoteSrcBlocked`
  // stops the renderer emitting a remote <img>/<video> src that the CSP would
  // only fail later, with no explanation the reader could act on.
  if (ISPO_PINNED_OFFLINE) return true
  return sessionOffline ?? lsGet('bento-offline') === 'on'
}

/** ISPO PORT: the host grants no network egress, and no switch can change it. */
export const ISPO_PINNED_OFFLINE = true
...
export const setOffline = (on: boolean): boolean => {
  // ISPO PORT: offline is a property of the host, not a preference. Turning it
  // OFF is refused and says so through the same return value the caller
  // already has to honour ("did this stick?") — the one rule this switch has
  // is that it must never claim a state it is not in.
  if (ISPO_PINNED_OFFLINE) {
    enforceOffline()
    return on
  }
  sessionOffline = on
