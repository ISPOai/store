...
export function useAssets(slideId: string): UseAssetsResult {
  const available = __OSD_ENV__.DEV;
  ...
  useEffect(() => {
    if (!available || !__OSD_HOT__) return;
    ...
    __OSD_HOT__.on('open-slide:assets-changed', assetHandler);
    __OSD_HOT__.on('open-slide:slide-changed', slideHandler);
    return () => {
      __OSD_HOT__?.off('open-slide:assets-changed', assetHandler);
      __OSD_HOT__?.off('open-slide:slide-changed', slideHandler);
    };
  }, [slideId, refresh]);
