...
export function Slide() {
  ...
  useEffect(() => {
    ...
    __OSD_HOT__.send('open-slide:current', {
      slideId,
      pageIndex: index,
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'f' || e.key === 'F') {
        ...
        setPlayMode('window');
      } else if (__OSD_ENV__.DEV && (e.key === 'd' || e.key === 'D')) {
        setDesignOpen((v) => !v);
      }
    };
        ...
        <div className="flex h-dvh flex-col overflow-hidden bg-sidebar text-foreground">
          ...
          <header className="relative flex h-12 shrink-0 items-center gap-2 px-2 md:px-3">
            <div className="flex flex-1 items-center gap-1.5 md:flex-none md:gap-2">
              ...
              {__OSD_ENV__.DEV && <AgentConnectedBadge />}
            </div>
              ...
              <div className="relative flex min-h-0 flex-1 flex-col">
                <div className="flex min-h-0 flex-1 flex-col md:flex-row">
                  <ResizableRail
                    ...
                    current={index}
                    onSelect={goTo}
                    onReorder={__OSD_ENV__.DEV ? reorderPage : undefined}
...
function SelectionReporter() {
  ...
  useEffect(() => {
    ...
    const selection = selected
      ...
      : null;
    __OSD_HOT__.send('open-slide:current', { selection });
  }, [selected]);
  return null;
}
