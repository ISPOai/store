...
type ListVideoProjectsInput = {
	limit?: number;
};

type ListVideoProjectsResult = {
	kind: "json";
	data: {
		count: number;
...
function isMissingProjectsDirectory(error: unknown): boolean {
	return errorMessage(error).includes("project directory not found:");
}

function isMissingProjectFile(error: unknown): boolean {
	return errorMessage(error).includes("project file not found:");
}
...
>(
	...
	async (input, ctx) => {
		...
		try {
			...
			if (!isMissingProjectsDirectory(error)) throw error;
		}
		...
		for (const entry of entries) {
			...
			try {
				...
				// A file can disappear between list and read. Permission, transport,
				// and other storage failures must remain visible to the caller.
				if (!isMissingProjectFile(error)) throw error;
			}
		}
